...
# Check if VNet is already running for the correct proxy
if pgrep -f "tsh.*--proxy $TSH_PROXY.*vnet" > /dev/null || pgrep -f "tsh.*vnet.*--proxy $TSH_PROXY" > /dev/null; then
	echo "tsh vnet is already running ($TSH_PROXY)"
	exit 0
fi

# Check if VNet is running for a different proxy
if pgrep -f "tsh.*vnet" > /dev/null; then
	echo "Error: VNet is running for a different proxy (need $TSH_PROXY)" >&2
	echo "Stop the current VNet first (sudo pkill -f 'tsh.*vnet'), then retry." >&2
	exit 1
fi
...
# Verify it started
if pgrep -f "tsh.*vnet" > /dev/null; then
	echo "tsh vnet started successfully ($TSH_PROXY)"
	exit 0
else
